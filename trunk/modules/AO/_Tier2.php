...
* Revision: $Id: Tier2.php 1554 2008-05-11 23:15:50Z temar $
*/
	...
	*/
	...
	function enter_all_raffles($name)
	{
		...
		foreach (array("embryo", "will", "advent", "will_ffa", "advent_ffa") as $cat) {
			if ($this->users[$name][$cat]) { continue; }
			$item = $cat;
			if ($cat == "will")
	...
	function reset_loot_list($group, $item)
	{
		foreach ($this -> users as $name => $data) {
			...
			if ($group == "advent") {
				if ($item != $this -> advents[$data[profession]]) { continue; }
			}
			if ($group == "will") {
				if ($item != $this -> wills[$data[profession]]) { continue; }
			}
