...
class City extends BaseActiveModule
{
	...
	*/
	...
	function __construct(&$bot)
	{
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS `city`
				(`key` VARCHAR(32) NOT NULL PRIMARY KEY,
				`vark` VARCHAR(255))");
		$result = $this -> bot -> db -> select("SELECT `vark` FROM `city` WHERE `key` = 'progress'");
		if (!empty($result))
		{
		...
		{
			$this -> bot -> db -> select("REPLACE INTO `city` (`key`, `vark`) VALUES ('progress', '" . implode(",",$this->progress) . "')");
		}
		
		$result = $this -> bot -> db -> select("SELECT `vark` FROM city WHERE `key` = 'stock'");
		if (!empty($result))
		{
		...
		{
			$this -> bot -> db -> select("REPLACE INTO `city` (`key`, `vark`) VALUES ('stock', '" . implode(",",$this->stock) . "')");
		}
