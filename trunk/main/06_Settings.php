...
*  USA
*
* File last changed at $LastChangedDate: 2008-05-22 17:33:39 +0100 (Thu, 22 May 2008) $
* Revision: $Id: 06_Settings.php 1599 2008-05-22 16:33:39Z alreadythere $
*/
...
{ // Start Class
	...
	*/
	...
	{ // Start function save()
		...
		$module_sql = addslashes(strval($module));
		$setting_sql = addslashes(strval($setting));
		$value_sql = addslashes(strval($value));

		if ($noupdate)
		{
			$sql = "INSERT IGNORE INTO #___settings (module, setting, value, datatype) VALUES ('".$module_sql."','".$setting_sql."','".$value_sql."','".$datatype."')";
		}
		else
		{
	...
	*/
	...
	{ // Start function create()
		...
		$defaultoptions = addslashes($defaultoptions);
		$sql = "INSERT INTO #___settings (module, setting, value, datatype, longdesc, defaultoptions, hidden, disporder) ";
		$sql .= "VALUES ('".$module."', '".$setting."', '".$valuesql."', '".$datatype."', '".$longdesc."', '".$defaultoptions."', ".$hidden.", ".$disporder.")";
		$result = $this -> bot -> db -> returnQuery($sql);
		// FIXME: INSERT IGNORE will always be a successful query, so maybe this needs to be redone.
		if (!$result)
		{
			$status['error'] = TRUE;
		...
		else
		{
			...
			if ($this -> get("Settings", "Log"))
			{
				$this -> bot -> log("SETTINGS", "SAVED", "Set ".stripslashes($setting)." for module ".stripslashes($module)." to ".$value);
			}
		}
		return $status;
	} // End function create()
	...
	*/
	...
	{ // Start function update()
		...
		if ($this -> get("Settings", "Log"))
		{
			$this -> bot -> log("SETTINGS", "UPDATE", "Set ".stripslashes($what)." for setting [".stripslashes($module)."][".stripslashes($setting)."] to ".stripslashes($to));
		}
	} // End function update()
	...
	*/
	...
	{ // Start function del()
		...
		if (is_null($setting)) // Delete a setting group.
		{
			...
			{
				$sql = "DELETE FROM #___settings WHERE module = '".addslashes($module)."'";
				$this -> bot -> db -> query($sql);
		...
		else
		{
			...
			$sql = "DELETE FROM #___settings WHERE module = '".addslashes($module)."' AND setting = '".addslashes($setting)."'";
			$this -> bot -> db -> query($sql);
			return $status;
		}
	} // End function del()
	...
	*/
	...
	{ // Start function check_data()
		...
		$check[2] = array("value", $value);
		$check[3] = array("longdesc", $longdesc);
		$check[4] = array("defaultoptions", $defaultoptions);
