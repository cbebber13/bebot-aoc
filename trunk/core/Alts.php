...
* Revision: $Id: Alts.php 1554 2008-05-11 23:15:50Z temar $
*/
...
class Alts_Core extends BasePassiveModule
{
	...
	*/
	function __construct(&$bot)
	{
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("alts", "false") . " (alt VARCHAR(255) NOT NULL PRIMARY KEY, main VARCHAR(255))");
	...
	*/
	function create_caches()
	{
		...
		$altlist = $this -> bot -> db -> select("SELECT main, alt FROM #___alts ORDER BY main ASC, alt ASC");
		if (empty($altlist))
		{
			return;
		}
			...
			foreach ($alts as $alt)
			{
				if($alt != $whois['nickname'])
				{
					...
					if ($this -> bot -> core("settings") -> get('Alts', 'Detail'))
					{
						...
						{
							$window .= "\n##normal## - (##highlight##" . $whoisalt['level'] . "##end## " . $whoisalt['class'] . ")##end##";
						}
						unset($whoisalt);
					}
