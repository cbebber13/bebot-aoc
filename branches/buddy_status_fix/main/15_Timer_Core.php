...
* Revision: $Id: 15_Timer_Core.php 1605 2008-05-22 21:22:48Z alreadythere $
*/
...
class Timer_Core extends BasePassiveModule
{
	...
	function __construct(&$bot)
	{
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("timer_class_entries", "false") . " (
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("timer_classes", "false") . " (
		...
		$this -> bot -> core("settings") -> create("Timer", "SchemaVersion", 1, "The version of the user table schema.", NULL, TRUE, 1);
		$this -> bot -> core("settings") -> create("Timer", "DefaultClass", "Standard", "Which is the name of the default class to use for timers if no other is specified?");
		$this -> bot -> core("settings") -> create("Timer", "GuestChannel", "origin", "Should an timer started in guestchannel or org-chat show up in both those channels or just in the channel of origin?", "both;origin");
		$this -> bot -> core("settings") -> create("Timer", "Global", FALSE, "Are all timers global? Notices for global timers are sent both to guild chat and private group. Global timers are listed in every timer listing too.");
		$this -> bot -> core("settings") -> create("Timer", "DeleteRank", "ADMIN", "Minimal access level to remove timer of other players.", "ANONYMOUS;GUEST;MEMBER;LEADER;ADMIN;SUPERADMIN;OWNER");
		$this -> bot -> core("settings") -> create("Timer", "Relay", FALSE, "Should new timer be relayed to all relay bots using the established relay network (See module/Relay.php for more informations about relaying)?");
		$this -> bot -> core("settings") -> create("Timer", "MinRepeatInterval", 30, "What's the minimal repeat interval for repeating timers (in seconds)? This is used to prevent annoying spam.", "5;10;15;20;25;30;45;60");
		$this -> bot -> core("settings") -> create("Timer", "RecoveryCheckInterval", 10, "How many minutes should pass between each check to recover after a database failures?", "5;10;15;30;45;60");
	...
	function update_timer_table()
	{
		...
		switch ($this -> bot -> db -> get_version("timer_class_entries"))
		{
			case 1:
				...
				$this -> bot -> db -> update_table("timer_class_entries", "notify_prefix", "add",
				...
				if (empty($res))
				{
					$this -> bot -> db -> query("UPDATE #___timer_class_entries SET notify_prefix = 'Timer' "
					. "WHERE notify_prefix = ''");
				}
		}
		...
		{
			case 1:
				$this -> bot -> db -> update_table("timer", "channel", "modify",
	...
	function check_timers()
	{
		...
		if ($this -> last_recovery_check + 60 * $this -> bot -> core("settings") -> get("Timer", "RecoveryCheckInterval") <= $thistime)
		{
			$this -> update_next_timer();
		}
			...
			foreach ($timers as $timer)
			{
				...
				if ($channel == 'internal')
				{
					...
					if (isset($this -> modules[strtolower($timer['owner'])]))
					{
						...
						$this -> modules[strtolower($timer['owner'])] -> timer($timer['name'], $this -> class_cache[$timerclass]['prefix'],
						$this -> class_cache[$timerclass]['suffix'], $this -> class_cache[$timerclass]['delay']);
					}
				}
				else
				{
				...
				{
					$this -> bot -> db -> query("UPDATE #___timer SET timerclass = "
					. $this -> class_cache[$timerclass]['next'] . " WHERE id = " . $timer['id']);
				}
				...
				elseif ($timer['repeatinterval'] > 0)
				{
					...
					{
						$entry = $this -> get_class_entry($this -> bot -> core("settings") -> get("Timer", "DefaultClass"), $timer['repeatinterval']);
					}
					else
					{
					...
					$this -> bot -> db -> query("UPDATE #___timer SET timerclass = " . $entry
					. ", endtime = endtime + repeatinterval WHERE id = " . $timer['id']);
				}
			}
	...
	{
		$classret = $this -> bot -> db -> select("SELECT t1.id FROM #___timer_class_entries AS t1, #___timer_classes AS t2"
		. " WHERE t1.class_id = t2.id AND t2.name = '" . $class . "' AND t1.notify_delay < " . $duration
		. " ORDER BY t1.notify_delay DESC LIMIT 1");
	...
	function add_timer($relaying, $owner, $duration, $name, $channel, $repeat, $class = "")
	{
		...
		if ($this -> bot -> core("settings") -> get("Timer", "GuestChannel") == "both"
		&& ($channel == "gc" || $channel == "pgmsg"))
		{
			$channel = "both";
		}
		if ($this -> bot -> core("settings") -> get("Timer", "Global") && $channel != "internal")
		{
			$channel = "global";
		}

		$this -> bot -> db -> query("INSERT INTO #___timer (name, timerclass, endtime, owner, channel, repeatinterval) VALUES "
		. "('" . mysql_real_escape_string($name) . "', " . $classid . ", " . $endtime . ", '"
		. mysql_real_escape_string($owner) . "', '" . mysql_real_escape_string($channel) . "', " . $repeat . ")");
		...
		if (!$relaying && $channel != "tell" && $channel != 'internal' && $this -> bot -> core("settings") -> get("timer", "relay")
	...
	function del_timer($deleter, $id, $silent = true)
	{
		...
		elseif ($this -> bot -> core("alts") -> main($deleter) == $this -> bot -> core("alts") -> main($deltimer[0][0]))
		{
			$dodelete = true;
		}
		elseif ($this -> bot -> core("security") -> check_access($deleter, $this -> bot -> core("settings") -> get("Timer", "DeleteRank")))
		{
			$dodelete = true;
			$admin = true;
		}
	...
	{
		return $this -> bot -> db -> select("SELECT id, name, timerclass, channel, owner, endtime, repeatinterval "
		. " FROM #___timer WHERE endtime <= " . $thistime
		. " UNION SELECT t1.id AS id, name, timerclass, channel, owner, endtime, repeatinterval "
		. " FROM #___timer AS t1 JOIN #___timer_class_entries AS t2 ON t1.timerclass <> -1 AND t1.timerclass = t2.id "
		. " WHERE endtime - notify_delay <= " . $thistime, MYSQL_ASSOC);
	}
	...
	{
		$this -> bot -> db -> query("INSERT IGNORE INTO #___timer_classes (name, description) VALUES ('"
		. mysql_real_escape_string($name) . "', '" . mysql_real_escape_string($description) . "')");
	...
	function create_timer_class_entry($class_id, $next_id, $delay, $prefix, $suffix)
	{
		$this -> bot -> db -> query("INSERT IGNORE INTO #___timer_class_entries (next_id, class_id, notify_delay, notify_prefix, "
		. "notify_suffix) VALUES (" . $next_id . ", " . $class_id . ", " . $delay . ", '" . mysql_real_escape_string($prefix) . "', '"
		. mysql_real_escape_string($suffix) . "')");

		$id = $this -> bot -> db -> select("SELECT id FROM #___timer_class_entries WHERE class_id = " . $class_id
		. " AND notify_delay = " . $delay);
		...
		{
			$this -> bot -> db -> query("UPDATE #___timer_class_entries SET next_id = " . $id[0][0] . " WHERE class_id = "
			. $class_id . " AND notify_delay = " . $delay);
		}
