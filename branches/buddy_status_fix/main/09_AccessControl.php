...
* Revision: $Id: 09_AccessControl.php 1572 2008-05-16 06:12:55Z temar $
*/
...
class AccessControl_Core extends BasePassiveModule
{
	...
	function __construct(&$bot)
	{
		...
		*/
		...
		$this -> bot -> core("settings") -> create("AccessControl", "DefaultLevel", "OWNER", "Which is the minimal access level that should get access to new commands on default? Or should the default for new commands be to disabled access to them at all?", "ANONYMOUS;GUEST;MEMBER;LEADER;ADMIN;SUPERADMIN;OWNER;DISABLED");
		...
		$this -> access_levels = array('ANONYMOUS', 'GUEST', 'MEMBER', 'LEADER', 'ADMIN', 'SUPERADMIN', 'OWNER', 'DISABLED', 'DELETED');
		$this -> security_levels = array('ANONYMOUS', 'GUEST', 'MEMBER', 'LEADER', 'ADMIN', 'SUPERADMIN', 'OWNER');
		$this -> deny_levels = array('DISABLED');
		$this -> channels = array("tell", "pgmsg", "gc", "extpgmsg", "all");
	...
	{
		if($this -> bot -> core("settings") -> exists("accesscontrol", "schemaversion"))
		{
			$this -> bot -> db -> set_version("access_control",
			$this -> bot -> core("settings") -> get("accesscontrol", "schemaversion"));
			$this -> bot -> core("settings") -> del("accesscontrol", "schemaversion");
		}
		switch ($this -> bot -> db -> get_version("access_control"))
		{
			case 1:
				$this -> bot -> db -> update_table("access_control", "subcommand", "add",
					"ALTER IGNORE TABLE #___access_control ADD COLUMN subcommand VARCHAR(50) NOT NULL DEFAULT '*' AFTER command");
				$this -> bot -> db -> query(
					"ALTER IGNORE TABLE #___access_control DROP PRIMARY KEY");
				$this -> bot -> db -> update_table("access_control", array("command", "subcommand", "channel"), "alter",
					"ALTER IGNORE TABLE #___access_control ADD PRIMARY KEY (command, subcommand, channel)");
				$this -> bot -> db -> query(
					"UPDATE #___access_control SET subcommand = '*' WHERE subcommand = ''");
			case 2:
				$this -> bot -> db -> update_table("access_control", "minlevel", "modify",
		...
		if ($this -> startup)
		{
			...
			for ($i = 0; $i < $channelcount; $i++)
			{
				if (!(empty($this -> bot -> commands[$channel[$i]])))
				{
					...
					{
						$rights = $this -> bot -> db -> select("SELECT * FROM #___access_control WHERE command = '"
							. $key . "' AND channel = '" . $channel[$i] . "'");

						if (empty($rights))
						{
							$this -> bot -> db -> query("INSERT INTO #___access_control (command, subcommand, channel, "
								. "minlevel) VALUES ('" . $key . "', '*', '" . $channel[$i] . "', '"
								. $this -> bot -> core("settings") -> get("Accesscontrol", "Defaultlevel") . "')");
						}
					}
				}
			}
	...
	function do_check($user, $command, $subcommand, $channel)
	{
		...
		else
		{
			...
			return $this -> bot -> core("security") -> check_access($user,
	...
	function check_rights($user, $command, $msg, $channel)
	{
		...
		if (($channel == "gc" && $this -> bot -> core("settings") -> get("AccessControl", "LockGc"))
		|| ($channel == "pgmsg" && $this -> bot -> core("settings") -> get("AccessControl", "LockPgroup")))
		{
			return FALSE;
		}
	...
	function update_access($command, $channel, $newlevel)
	{
		...
		$this -> bot -> db -> query("INSERT INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
				. "', '*', '" . $channel . "', '" . $newlevel . "') ON DUPLICATE KEY UPDATE minlevel = '" . $newlevel . "'");
	...
	function create($channel, $command, $defaultlevel)
	{
		...
		{
			$this -> bot -> log("ACCESS", "ERROR", "Trying to set an illegal default level of " . $defaultlevel
		...
		{
			$this -> bot -> log("ACCESS", "ERROR", "Trying to set default access rights for an illegal channel: "
			. $channel . " for " . $command . "!");
			return;
		}
		...
		if ($channel == "all")
		{
			$chans = array("tell", "pgmsg", "gc");
			foreach ($chans as $chan)
			{
				// Add the default level if no entry for the channel/command combination exists yet:
				$this -> bot -> db -> query("INSERT IGNORE INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('"
				. $command . "', '*', '" . $chan . "', '" . $defaultlevel . "')");
		...
		{
			// Add the default level if no entry for the channel/command combination exists yet:
			$this -> bot -> db -> query("INSERT IGNORE INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
	...
	function update($command, $sub, $channel, $newlevel)
	{
		...
		{
			$this -> bot -> log("ACCESS", "ERROR", "Trying to set an illegal default level of " . $newlevel
				. " for " . $command . " with subcommand " . $sub . " in " . $channel . "!");
			return;
		}

		if ($newlevel == 'DELETED' && $sub == '*')
		{
			$this -> bot -> log("ACCESS", "ERROR", "You cannot delete commands (subcommand * entries)! "
				. "Command: " . $command . " with subcommand " . $sub . " in " . $channel . "!");
			return;
		}

		// Add access level if not yet existing, otherwise update existing entry:
		$this -> bot -> db -> query("INSERT INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
				. "', '" . $sub . "', '" . $channel . "', '" . $newlevel . "') ON DUPLICATE KEY UPDATE minlevel = '" . $newlevel . "'");
	...
	function create_subcommand($channel, $command, $sub, $defaultlevel)
	{
		...
		{
			$this -> bot -> log("ACCESS", "ERROR", "Trying to set default access rights for an illegal channel: "
			. $channel . " with subcommand " . $sub . " for " . $command . "!");
			return;
		}

		if ($sub == "*")
		{
			$this -> bot -> log("ACCESS", "ERROR", "You cannot set access levels for the general subcommand * "
			. "using the create_subcommand() function! Use create() for that! Command " . $command . "!");
			return;
		}
	...
	function save($name, $desc)
	{
		$count = 0;
		$countsub = 0;
		$cshort = array("tell" => "t", "gc" => "g", "pgmsg" => "p", "extpgmsg" => "e");
		$lshort = array("ANONYMOUS" => "AN", "GUEST" => "G", "MEMBER" => "M", "LEADER" => "L", "ADMIN" => "A",
					"SUPERADMIN" => "SA", "OWNER" => "O", "DISABLED" => "D");
		...
		$save = implode(";", $coms);
		$this -> bot -> db -> query("INSERT INTO #___access_control_saves (name, description, commands) VALUES ('".mysql_real_escape_string($name)."', '".mysql_real_escape_string($desc)."', '".$save."')");
		Return (array($count, $countsub));
	}
	...
	function load($name)
	{
		$count = 0;
		$countsub = 0; 	
		$clong = array("t" => "tell", "g" => "gc", "p" => "pgmsg", "e" => "extpgmsg");
		$llong = array('AN' => 'ANONYMOUS', 'G' => 'GUEST', 'M' => 'MEMBER', 'L' => 'LEADER', 'A' => 'ADMIN',
					'SA' => 'SUPERADMIN', 'O' => 'OWNER', 'D' => 'DISABLED');
			...
			foreach($commands as $command)
			{
				...
				foreach($subcommands as $subcommand)
				{
					...
					foreach($channels as $channel)
					{
						...
						if($chan == "a")
						{
							$load[] = array($com, $subcom, "tell", $llong[$lvl]);
							$load[] = array($com, $subcom, "gc", $llong[$lvl]);
							$load[] = array($com, $subcom, "pgmsg", $llong[$lvl]);
						}
						elseif($chan == "r")
						{
							$load[] = array($com, $subcom, "tell", $llong[$lvl]);
							...
							$load[] = array($com, $subcom, $clong[$chan], $llong[$lvl]);
					}
				}
			}
