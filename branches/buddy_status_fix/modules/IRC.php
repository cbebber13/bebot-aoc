...
* Revision: $Id: IRC.php 1554 2008-05-11 23:15:50Z temar $
*/
...
class IRC extends BaseActiveModule
{
	...
	*/
	function __construct(&$bot)
	{
		...
		// Update AnnounceWhat setting to the current values:
		$this -> bot -> core("settings") -> update("Irc", "AnnounceWhat", "longdesc", "Which events should be announced to IRC? None, only  buddies logging on, only joins to the chatgroup or both?", "none;buddies;joins;both");
		$this -> bot -> core("settings") -> update("Irc", "AnnounceWhat", "defaultoptions", "none;buddies;joins;both");
	...
	function gmsg($name, $group, $msg)
	{
		...
		|| (strtolower($this -> bot -> core("settings") -> get("Irc", "Chat")) == "both")))
		{
			if (!$this -> send_irc($this -> bot -> core("settings") -> get("Irc", "Ircguildprefix"), $name, $msg))
			{
				...
				{
					$this -> bot -> send_gc("##error##Last line not relayed to IRC as it's containing too many characters!##end##");
				}
			}
		}
	}
	...
	function privgroup($name, $msg)
	{
		...
		|| (strtolower($this -> bot -> core("settings") -> get("Irc", "Chat")) == "both")))
		{
			if (!$this -> send_irc($this -> bot -> core("settings") -> get("Irc", "Ircguestprefix"), $name, $msg))
			{
				if ($this -> bot -> core("settings") -> get("Irc", "Notifyondrop"))
				{
					$this -> bot -> send_pgroup("##error##Last line not relayed to IRC as it's containing too many characters!##end##");
				}
				return;
			}
		}
