...
*  USA
*
* File last changed at $LastChangedDate: 2008-05-23 15:27:17 +0100 (Fri, 23 May 2008) $
* Revision: $Id: 03_Security.php 1617 2008-05-23 14:27:17Z alreadythere $
*/
...
$security = new Security_Core($bot, $owner, $super_admin);
...
{ // Start Class
	...
	*/
	function __construct(&$bot, $owner, $super_admin)
	{
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("security_groups", "true") . "
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("security_members", "true") . "
		...
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS ".$this -> bot -> db -> define_tablename("security_org", "true")."
		...
		$this -> owner = ucfirst(strtolower($owner));
		$this -> super_admin = array();
		if (!empty($super_admin))
		{
			foreach ($super_admin as $user => $value)
			{
	...
	*/
	...
	{ // Start function enable()
		...
		$this -> bot -> core("settings") -> create("Security", "UseAlts", FALSE, $longdesc);
		$longdesc = "Should all characters in the chat group of the bot be considered GUESTs for security reasons?";
		$this -> bot -> core("settings") -> create("Security", "GuestInChannel", TRUE, $longdesc);
		$longdesc = "Security Access Level required to add members and guests.";
		$this -> bot -> core("settings") -> create("Security", "adduser", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 1);
		$longdesc = "Security Access Level required to remove members and guests.";
		$this -> bot -> core("settings") -> create("Security", "deluser", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 2);
		$longdesc = "Security Access Level required to add security groups.";
		$this -> bot -> core("settings") -> create("Security", "addgroup", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 3);
		$longdesc = "Security Access Level required to remove security groups.";
		$this -> bot -> core("settings") -> create("Security", "delgroup", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 4);
		$longdesc = "Security Access Level required to add users to security groups.";
		$this -> bot -> core("settings") -> create("Security", "addgroupmember", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 5);
		$longdesc = "Security Access Level required to remove users from security groups.";
		$this -> bot -> core("settings") -> create("Security", "remgroupmember", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 6);
		$longdesc = "Security Access Level required to use <pre>security whois name.";
		$this -> bot -> core("settings") -> create("Security", "whois", "leader", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 10);
		$longdesc = "Security Access Level required to change settings for all modules.";
		$this -> bot -> core("settings") -> create("Security", "settings", "SUPERADMIN", $longdesc, "OWNER;SUPERADMIN;ADMIN;LEADER", FALSE, 99);

		$this -> enabled = TRUE;
		$this -> last_alts_status = $this -> bot -> core("settings") -> get("Security", "UseAlts");
		$this -> last_leader = "";
	} // End function enable()
	...
	{ // Start funciton handler
		...
		switch($command)
		{
			case 'security':
				switch($vars[1])
				{
					case "changelevel":
						...
						{
							return "Only SUPERADMINs can access the changelevel command.";
						}
					break;
					case "levels":
						if ($this -> check_access($source, "SUPERADMIN"))
						...
						{
							return "Only SUPERADMINs can modify the access levels.";
						}
					case "whois":
						if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Whois')))
						{
							return $this -> whois($vars[2]);
						}
						else
						{
							return "You must have ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Whois'))." access or higher to use <pre>security whois";
						}
					break;
					case "whoami":
			...
			case 'adduser': // adduser username group
				if (isset($vars[2]))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Addgroupmember')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Addgroupmember'))."s and above can add group members.";
					}
				}
				else
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Adduser')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Adduser'))."s and above can add users.";
					}
				}
				break;
			case 'deluser':
				if (isset($vars[2]))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Remgroupmember')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Remgroupmember'))."s and above can remove group members.";
					}
				}
				else
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Deluser')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Deluser'))."s and above can remove users.";
					}
				}
				break;
			case 'addgroup':
				if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Addgroup')))
				{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Not enough paramaters given. Try /tell <botname> <pre>addgroup groupname description.";
					}
				}
				else
				{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Addgroup'))."s and above can add groups.";
				}
				break;
			case 'delgroup':
				if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Delgroup')))
				{
					if (isset($vars[1]))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Not enough paramaters given. Try /tell <botname> <pre>delgroup groupname.";
					}
				}
				else
				{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Delgroup'))."s and above can delete groups.";
				}
				break;
			case 'admin':
				...
				else if (preg_match("/^admin group (remove|rem|del) ([a-zA-Z0-9]+)$/i", $msg, $info))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Delgroup')))
					{
				...
				else if (preg_match("/^admin add ([a-zA-Z0-9]+) ([a-zA-Z0-9]+)$/i", $msg, $info))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Addgroupmember')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Addgroupmember'))."s and above and add group members.";
					}
				}
				else if (preg_match("/^admin (remove|rem|del) ([a-zA-Z0-9]+) ([a-zA-Z]+)$/i", $msg, $info))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Remgroupmember')))
					{
				...
				else if (preg_match("/^admin (remove|rem|del) ([a-zA-Z0-9]+)$/i", $msg, $info))
				{
					if ($this -> check_access($source, $this -> bot -> core("settings") -> get('Security', 'Deluser')))
					{
					...
					{
						$return['error'] = TRUE;
						$return['errordesc'] = "Only ".strtoupper($this -> bot -> core("settings") -> get('Security', 'Deluser'))."s and above can delete users.";
					}
				}
				else
				{
	...
	*/
	...
	{ // Start function add_group()
		...
		if ($groupname == "leader" || $groupname == "admin" || $groupname == "superadmin" || $groupname == "member"
			|| $groupname == "guest" || $groupname == "anonymous" || $groupname == "owner")
		{
			$return['error'] = TRUE;
			$return['errordesc'] = ucfirst($groupname)." is a default group and cannot be created as a custom group.";
			$return['content'] = ucfirst($groupname)." is a default group and cannot be created as a custom group.";
			return $return;
		}
		// Check for bad input.
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Group Names should not be all numbers.";
			$return['content'] = "Group Names should not be all numbers.";
			return $return;
		}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Group Names should be five or more characters.";
			$return['content'] = "Group Names should be five or more characters.";
			return $return;
		}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Single quotes, double quotes, backslash and other special characters are not allowed in group names.";
			$return['content'] = "Single quotes, double quotes, backslash and other special characters are not allowed in group names.";
			return $return;
		}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Group ".$groupname." already exisits";
			$return['content'] = "Group ".$groupname." already exisits";
			return $return;
		}
		...
		$gid = $result[0][0];
		unset($result);
		$tmp = array("gid" => $gid, "name" => $groupname, "description" => $description, "access_level" => 0);
		$tmp['members'] = array();
		$this -> cache_mgr("add", "groups", $tmp);
		$return['error'] = FALSE;
		$return['errordesc'] = '';
		$return['content'] = "Created group ".$groupname." with anonymous level privlages.";
		$this -> bot -> log("SECURITY", "ADDGROUP", $caller." ".$return['content']);
		return $return;
	...
	*/
	...
	{ // Start function del_group()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." cannot be deleted.";
			$return['content'] = $target." cannot be deleted.";
			return $return;
		}
		...
		if (is_numeric($target))
		{
			...
			{
				$return['error'] = TRUE;
				$return['errordesc'] = "Group ID ".$target." not found.";
				$return['content'] = "";
				return $return;
			}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Group ".$target." does not exisit";
			$return['content'] = "";
			return $return;
		}
		...
		$this -> bot -> db -> query($sql);
		$return['content'] = "Deleted group ID ".$this -> cache['groups'][$target]['gid']." Name: ".$target.".";
		$this -> bot -> log("SECURITY", "DELGROUP", $caller." ".$return['content']);
	...
	*/
	...
	{ // Start function add_group_member()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is not a valid character.";
			$return['content'] = '';
			return $return;
		}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = "Unable to find group ID for ".$group." ".$group." may not exisit. Check your spelling and try again.";
			$return['content'] = '';
			return $return;
		}
		...
		if (!isset($this -> cache['groups'][$gid]['members'][$target]))
		{
			...
			$return['content'] = "Added ".$target." to group ".$group.".";
			$this -> bot -> log("SECURITY", "GRPMBR", $caller." ".$return['content']);
		}
		else
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is already a member of ".$group;
			$return['content'] = '';
		}
		return $return;
	...
	*/
	...
	if ($gid == -1)
	{
		...
		if (!preg_match("/^Internal Process: (.*)$/i", $caller))
		{
			...
			{
				$return['error'] = TRUE;
				$return['errordesc'] = "Your Access Level is lower than ".$group."'s Access Level. You cannot remove members from ".$group.".";
				return $return;
			}
		}
		...
		if (isset($this -> cache['groups'][$group]['members'][$target]))
		{
			...
			$return['content'] = "Removed ".$target." from ".$group;
			$this -> bot -> log("SECURITY", "GRPMBR", $caller." ".$return['content']);
		}
		else
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is not a member of ".$group;
		}
		return $return;
	...
	*/
	...
	{ // Start function add_user()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is banned.";
			return $return;
		}

		// Get whois data & check for errors.
// REMOVED BY NOER FOR AOC
/*
		$who = $this -> bot -> core("whois") -> lookup($target);
		if ($who['error'])
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $who['errordesc'];
			return $return;
		}
*/
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = ucfirst($target)." is already a ".$level.".";
			return $return;
		}
		else
		{
			...
			$sql .= "VALUES (".$who['id'].", '".$who['nickname']."', '".mysql_real_escape_string($admin)."', ".time().", ".$lvlnum.", ".time().") ";
			$sql .= "ON DUPLICATE KEY UPDATE added_by = '".mysql_real_escape_string($admin)."', added_at = ".time().", user_level=".$lvlnum.", updated_at = ".time();
			$this -> bot -> db -> query($sql);
			$return['content'] = "Added ".$target." as a ".$level;
			$this -> bot -> log("SECURITY", "ADDUSER", $admin." ".$return['content']);
			return $return;
		}
	...
	*/
	...
	{ // Start function del_user()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is not a member of <botname>.";
			return $return;
		}
		else
		{
			...
			$sql = "UPDATE #___users SET user_level = 0, deleted_by = '".mysql_real_escape_string($admin)."', deleted_at = ".time()." WHERE nickname = '".$target."'";
			$this -> bot -> db -> query($sql);
			$return['content'] = $target." has been removed from <botname>.";
			$this -> bot -> log("SECURITY", "DELUSER", $admin." ".$return['content']);
			return $return;
		}
	...
	$target = person being banned.
	*/
	function set_ban($admin, $target, $caller="Internal Process", $reason = "None given.", $endtime = 0)
	{ // Start function set_ban()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is the bot owner and cannot be banned.";
			return $return;
		}
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is already banned.";
			return $return;
		}
		elseif(isset($this -> cache['guests'][$target]))
		{
			...
			$sql = "UPDATE #___users SET user_level = -1, banned_by = '".mysql_real_escape_string($admin)."', banned_at = ".time().", banned_for = '".mysql_real_escape_string($reason)."', banned_until = ".$endtime." WHERE nickname = '".$target."'";
		}
		elseif(isset($this -> cache['members'][$target]))
		{
			$this -> cache_mgr("rem", "members", $target);
			$this -> cache_mgr("add", "banned", $target);
			$sql = "UPDATE #___users SET user_level = -1, banned_by = '".mysql_real_escape_string($admin)."', banned_at = ".time().", updated_at = ".time().", banned_for = '".mysql_real_escape_string($reason)."', banned_until = ".$endtime." WHERE nickname = '".$target."'";
		}
		else // They are not in the member table at all.
		{
			...
			$sql .= "VALUES ('".$who['id']."', '".$who['nickname']."', '".mysql_real_escape_string($admin)."', ".time().", '".mysql_real_escape_string($admin)."', ".time().", '".mysql_real_escape_string($reason)."', ".$endtime.", 0, -1, ".time().") ";
		...
		$return['content'] = "Banned ".$target." from " . $this -> bot -> botname . ".";
		$this -> bot -> log("SECURITY", "BAN", $caller." ".$return['content']);
		return $return;
	...
	*/
	...
	{ // Start function rem_ban()
		...
		{
			$return['error'] = TRUE;
			$return['errordesc'] = $target." is not banned.";
			return $return;
		}
		else
		{
			...
			$this -> bot -> db -> query($sql);
			$return['content'] = "Unbanned ".$target." from " . $this -> bot -> botname . ". ".$target." is now anonymous.";
			$this -> bot -> log("SECURITY", "BAN", $caller." ".$return['content']);
			return $return;
		}
	...
	*/
	...
	{ // Start function show_security_menu
		...
		$inside .= "[".$this -> bot -> core("tools") -> chatcmd("security groups", "Security Groups")."]\n";
		$inside .= "[".$this -> bot -> core("tools") -> chatcmd("security levels", "Security Levels")."]\n";
		$inside .= "[".$this -> bot -> core("tools") -> chatcmd("settings security", "Security Settings")."]\n";
		$inside .= "[".$this -> bot -> core("tools") -> chatcmd("security whoami", "Your Security Level and Group Membership")."]\n";
		$inside .= "\n";
	...
	*/
	...
	{ // Start function show_security_levels()
		...
		if ($this -> bot -> guildbot)
		{
			...
			$sql .= "WHERE org_gov = '".$this -> bot -> core("settings") -> get('Security', 'Orggov')."' ";
		...
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security", "Security: Main Menu")."] ";
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security groups", "Security: Security Groups")."] ";
		$return .= "\n\n";
		$return .= $superadmin."\n";
		$return .= $admin."\n";
		$return .= $leader."\n";
		...
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security", "Security: Main Menu")."] ";
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security groups", "Security: Security Groups")."] ";
		return $this -> bot -> core("tools") -> make_blob("Security Access Levels", $return);
	...
	{ // Start function show_groups()
		...
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security", "Security: Main Menu")."] ";
		$return .= "[".$this -> bot -> core("tools") -> chatcmd("security levels", "Security: Access Levels")."] ";
		$return .= "\n\n";
		$return .= $superadmins."\n";
		$return .= $admins."\n";
		$return .= $leaders."\n";
	...
	*/
	...
	{ // Start function change_links
		...
		if (is_array($groupid))
		{
			$vars = $this -> bot -> core("settings") -> get('Security', 'Orggov')." ".$groupid['org_rank'];
		}
		else
		{
			$vars = $groupid;
		...
		if ($levelid <> SUPERADMIN)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.SUPERADMIN, "SUPERADMIN", $msgtype)."] ";

		if ($levelid <> ADMIN)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.ADMIN, "ADMIN", $msgtype)."] ";

		if ($levelid <> LEADER)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.LEADER, "LEADER", $msgtype)."] ";

		if ($levelid <> MEMBER)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.MEMBER, "MEMBER", $msgtype)."] ";

		if ($levelid <> GUEST)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.GUEST, "GUEST", $msgtype)."] ";

		if ($levelid <> ANONYMOUS)
			$return .= "[".$this -> bot -> core("tools") -> chatcmd($chatcmd.ANONYMOUS, "ANONYMOUS", $msgtype)."] ";
		return $return;
	...
	*/
	...
	{ // Start function change_level()
		...
		elseif (strtolower($government) == strtolower($this -> bot -> core("settings") -> get('Security', 'Orggov')))
		{
			$orgrank = TRUE;
			$sql = "UPDATE #___security_org SET access_level = ".$newacl." WHERE org_gov = '".mysql_real_escape_string($government)."' AND org_rank = '".mysql_real_escape_string($groupid)."'";
		...
		if ($this -> bot -> db -> returnQuery($sql)) // Success
		{
			...
			{
				$tmp = array('gid' => $groupid, 'name' => $this -> cache['groups'][$groupid]['name']);
	...
	*/
	...
	{ // Start function get_group_name()
		...
		if (empty($result))
			...
			return $result[0]; // Should return an array with elements named id, name, description, and access_level
	...
	*/
	...
	if (!$this -> bot -> core("chat") -> get_uid($player))
		...
		if (isset($this -> cache['guests'][$player]))
			$highestlevel = 1;
		if ($this -> bot -> core("settings") -> get("Security", "GuestInChannel") && $this -> bot -> core("online") -> in_chat($player))
		{
			$highestlevel = 1;
	...
	if ($this -> bot -> core("flexible_security") != NULL)
	{
		$highestlevel = $this -> bot -> core("flexible_security") -> flexible_group_access($player, $highestlevel);
	}

	// !leader handling
	if ($this -> bot -> core("settings") -> exists("Leader", "Name") && $highestlevel < LEADER)
	{
		if ($this -> bot -> core("settings") -> get("Leader", "Leaderaccess")
			&& strtolower($player) == strtolower($this -> bot -> core("settings") -> get("Leader", "Name")))
		{
			$highestlevel = LEADER;
		}
	}
	...
	*/
	function get_access_level($player)
	{
		...
		if ($this -> last_alts_status != $this -> bot -> core("settings") -> get("Security", "UseAlts"))
		{
		...
		{
			if ($this -> bot -> core("settings") -> get("Leader", "Leaderaccess")
			&& strtolower($this -> last_leader) != strtolower($this -> bot -> core("settings") -> get("Leader", "Name")))
			{
				...
				{
					$leadername = $this -> bot -> core("alts") -> main($this -> last_leader);
				}
				unset($this -> cache['mains'][$leadername]);
				$this -> last_leader = $this -> bot -> core("settings") -> get("Leader", "Name");
				if (!$this -> bot -> core("settings") -> get("Security", "Usealts"))
				{
					$leadername = $this -> last_leader;
				}
		...
		// Check mains cache
		if (isset($this -> cache['mains'][$this -> bot -> core("alts") -> main($player)]))
		{
			return $this -> cache['mains'][$this -> bot -> core("alts") -> main($player)];
		}
	...
	*/
	...
	{ // Start function org_rank_access()
		$who = $this -> bot -> core("whois") -> lookup($player, TRUE); // Do whois with no XML lookup, guild members should be cached...
		if ($who['error'])
			return $highest;
	...
	*/
	...
	{ // Start function check_access()
		if (!$this -> enabled)
			return FALSE; // No access is granted until the secuirty subsystems are ready.
		$name = ucfirst(strtolower($name));
		$level = strtoupper($level);
		if ($level == "RAIDLEADER")
		{
	...
	*/
	...
	{ // Start set_government()
		...
		if (!($this -> bot -> core("settings") -> exists('Security', 'Orggov')))
			$this -> bot -> core("settings") -> create ("Security", "orggov", "Unknown", "Orginization Government Form", "Anarchism;Department;Faction;Feudalism;Monarchy;Republic;Unknown", TRUE, 99);
		$sql = "SELECT DISTINCT org_rank_id,org_rank FROM #___whois WHERE org_name = '".mysql_real_escape_string($this -> bot -> guildname)."' ORDER BY org_rank_id ASC"; // Gets the org ranks.
		$result = $this -> bot -> db -> select($sql);
		if (empty($result))
		{
		...
		if ($this -> bot -> core("settings") -> get('Security', 'Orggov') <> $orggov) // Change detected, reset access levels.
		{
			$this -> bot -> core("settings") -> save("Security", "orggov", $orggov);
	...
	{ // Start function cache_mgr()
		...
		switch ($cache)
		{
			case "guests":
				...
				unset($this -> cache['mains'][$this -> bot -> core("alts") -> main($info)]);
			break;
			case "members":
				if ($action)
				{
			...
			case "groups":
				if ($action)
				{
					...
					else // Updating a groups access level.
					{
						...
						foreach ($this -> cache['groups'][$info['gid']]['members'] as $member)
						{
							unset($this -> cache['mains'][$this -> bot -> core("alts") -> main($member)]);
						}
					}
				}
				else
	...
	*/
	...
	{ // Start function cache_org_ranks()
		...
		$sql = "SELECT org_rank, access_level FROM #___security_org ";
		$sql .= "WHERE org_gov = '".$this -> bot -> core("settings") -> get('Security', 'Orggov')."' ";
