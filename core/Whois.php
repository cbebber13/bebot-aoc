...
class Whois_Core extends BasePassiveModule
{
		...
		*/
		$this -> bot -> db -> query("CREATE TABLE IF NOT EXISTS " . $this -> bot -> db -> define_tablename("whois", "false")
				. " (
					...
					craft1 enum('','Alchemist','Architect','Armorsmith','Gemcutter','Weaponsmith'),
					craft2 enum('','Alchemist','Architect','Armorsmith','Gemcutter','Weaponsmith'),
		...
		$this -> bot -> core("settings") -> create('Whois', 'LookupOrder', 'funcom_auno', 'Which order should we use to look up players information?', 'funcom_auno;auno_funcom');
	...
	function update_table()
	{
		...
		switch ($this -> bot -> db -> get_version("whois"))
		{
			case 1: // Update Table version to prevent repeat update calls
				$this -> bot -> core("settings") -> update("Whois", "LookupOrder", "defaultoptions", "funcom_auno;auno_funcom;funcom_only;auno_only");
			case 2:
			default:
		}
	...
	*/
	...
	function whois_details($source, $whois)
	{
		...
		{
			$window .= "\n##normal##::: Options :::##end##\n";
			$window .= $this -> bot -> core("tools") -> chatcmd('addbuddy ' . $whois['nickname'], 'Add to buddylist') . "\n";
			$window .= $this -> bot -> core("tools") -> chatcmd('rembuddy ' . $whois['nickname'], 'Remove from buddylist') . "\n";
